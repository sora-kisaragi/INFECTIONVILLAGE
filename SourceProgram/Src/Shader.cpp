...
namespace Shader {
	...
	*/
	void Program::Reset(GLuint programId)
	{
		...
		locColor = glGetUniformLocation(id, "alpha");
	...
	* この関数を使う前に、Use()を実行しておくこと.
	*/
	void Program::Draw(const Mesh& mesh, const glm::vec3& t, const glm::vec3& r, const glm::vec3& s, float alpha)
	{
		if (id == 0) {
			return;
		}
